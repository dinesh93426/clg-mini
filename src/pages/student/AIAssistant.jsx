    ...
    <div className="h-[calc(100vh-8.5rem)] flex gap-5 overflow-hidden">
      ...
      <aside className="w-60 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl p-3.5 hidden md:flex flex-col gap-3 shadow-xs shrink-0">
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-1">
          <div className="p-2.5 rounded-lg bg-[#FFF1EB] text-[#FF5A1F] border border-[#FFD2C2] text-xs font-semibold cursor-pointer">
            <span className="block truncate">Campus Events Inquiry</span>
            <span className="text-[10px] text-[#FF5A1F]/80 block mt-0.5 font-normal">Active thread</span>
          </div>
      ...
      <div className="flex-1 flex flex-col bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl shadow-xs overflow-hidden relative min-w-0">
        ...
        <div className="px-5 py-3 border-b border-[#E2E8F0] flex items-center justify-between bg-[#FFFFFF]">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-[#FFF1EB] text-[#FF5A1F] flex items-center justify-center">
              <Bot size={16} />
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            return (
              ...
              >
                <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center text-xs font-semibold
                  ${isAI 
                    ? 'bg-[#EEECFF] text-[#FF5A1F] border border-[#FFD2C2]' 
                      ...
                      <div className="flex flex-wrap gap-1">
                        {msg.sources.map((src, sidx) => (
                          <button
                            ...
                            className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-[#FFFFFF] border border-[#E2E8F0] hover:border-[#FF5A1F] hover:bg-[#FFF1EB] text-[#FF5A1F] flex items-center gap-1 transition-colors cursor-pointer"
                          >
                            <BookOpen size={10} className="text-[#FF5A1F]" />
                            <span>{src.title}</span>
                          </button>
                        ))}
                      </div>
          ...
          {generating && (
            <div className="flex gap-2.5 mr-auto max-w-[80%]">
              <div className="w-7 h-7 rounded-full bg-[#FFF1EB] text-[#FF5A1F] border border-[#FFD2C2] flex items-center justify-center">
                <Bot size={14} />
              </div>
              <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl rounded-tl-none text-xs text-[#64748B] flex items-center gap-2">
                <span className="flex h-1.5 w-1.5 rounded-full bg-[#FF5A1F] animate-ping"></span>
          ...
          <div className="flex flex-wrap gap-1.5 py-0.5">
            {[
              ...
              <button
                ...
                disabled={generating}
                className="text-[11px] font-medium px-2.5 py-0.5 rounded-md bg-[#FFFFFF] border border-[#E2E8F0] hover:border-[#FF5A1F] hover:bg-[#FFF1EB] text-[#64748B] hover:text-[#FF5A1F] transition-colors cursor-pointer"
              >
                {p}
              </button>
            ))}
        ...
        >
          <input
            ...
            disabled={generating}
            className="flex-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3.5 py-2 text-xs text-[#172033] placeholder-[#94A3B8] focus:outline-none focus:border-[#FF5A1F] transition-colors"
          />
          <button
            type="submit"
            disabled={!inputValue.trim() || generating}
            className="p-2 rounded-lg bg-[#FF5A1F] hover:bg-[#E94712] text-white disabled:opacity-50 transition-colors cursor-pointer shadow-xs"
          >
            <Send size={14} />
          </button>
        </form>
      ...
      {activeContextEvent && (
        <aside className="w-68 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl p-4 hidden lg:flex flex-col justify-between overflow-y-auto shadow-xs shrink-0">
          <div className="space-y-3">
            <span className="text-[9px] text-[#FF5A1F] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-[#FFF1EB] inline-block border border-[#FFD2C2]">
              Contextual Event
            </span>
            <img 
            ...
            />
            <div>
              <span className="text-[9px] text-[#FF5A1F] font-bold uppercase tracking-wider">{activeContextEvent.category}</span>
          ...
          <div className="pt-3 border-t border-[#E2E8F0]">
            <Link
              ...
              className="w-full py-2 rounded-lg bg-[#FF5A1F] hover:bg-[#E94712] text-white text-xs font-semibold transition-colors shadow-xs flex items-center justify-center gap-1 cursor-pointer"
            >
