...
def get_sentiment_analytics() -> Dict[str, Any]:
    ...
    overall_rows = execute_query("""
        SELECT
            ...
            COUNT(*) FILTER (WHERE sentiment = 'POSITIVE' OR (sentiment IS NULL AND rating >= 4)) as pos_count,
            COUNT(*) FILTER (WHERE sentiment = 'NEUTRAL' OR (sentiment IS NULL AND rating = 3)) as neu_count,
            COUNT(*) FILTER (WHERE sentiment = 'NEGATIVE' OR (sentiment IS NULL AND rating <= 2 AND rating > 0)) as neg_count
        FROM "Feedback";
    """)
    ...
    event_sentiment_rows = execute_query("""
        SELECT
            e.id as event_id,
            e.title,
            e.category,
    ...
    monthly_rows = execute_query("""
        SELECT
            ...
            COUNT(*) as total_fb,
            COUNT(*) FILTER (WHERE sentiment = 'POSITIVE' OR (sentiment IS NULL AND rating >= 4)) as pos_c,
            COUNT(*) FILTER (WHERE sentiment = 'NEUTRAL' OR (sentiment IS NULL AND rating = 3)) as neu_c,
            COUNT(*) FILTER (WHERE sentiment = 'NEGATIVE' OR (sentiment IS NULL AND rating <= 2 AND rating > 0)) as neg_c
        FROM "Feedback" f
