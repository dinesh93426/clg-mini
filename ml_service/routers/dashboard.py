...
def dashboard_sentiment_endpoint():
    """
...
@router.get("/alerts")
def dashboard_alerts_endpoint(organizerId: Optional[str] = Query(None)):
    """
    Returns categorized early warning risk alerts (Critical, High, Medium, Low).
    """
    try:
        return {"alerts": get_early_warning_alerts(organizer_id=organizerId)}
    except Exception as e:
